...
sig = typing.Callable[[DDLSession], None]


async def run_migration_online(sess: DDLSession, upgrade: sig):
    """
...
Message: {message}
"""
from asyncqlio import DDLSession
...
async def _do_migrate(interface: DatabaseInterface, current_revision: int, revision: int):
    ...
    for migration in tqdm.tqdm(iterable=slice, desc="Migrating", unit="migrations"):  # type: Path
        ...
        async with interface.get_ddl_session() as sess:
